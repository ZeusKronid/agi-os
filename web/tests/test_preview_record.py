...
class GapStorageTests(unittest.TestCase):
    ...
    def test_mark_partition_only_for_labelled_unmounted_preview(self):
        ...
        snapshot = {'disks': [{'path': '/dev/vda', 'partitions': [
            {'path': str(self.device), 'partlabel': 'AGIOS-PREVIEW', 'mounted': False, 'size': 1, 'start': 2048},
        ...
        with patch.object(storage_worker, 'inventory', return_value=snapshot), \
            ...
            self.assertTrue(storage_worker.mark({'record': record, 'revert': {'kind': 'partition', 'device': str(self.device)}})['marked'])
            with self.assertRaises(ValidationError):
    ...
    def test_mark_file_writes_sidecar_inside_the_mounted_medium_only(self):
        storage_worker.PREVIEW = Path(self.directory.name) / 'preview'
        self.addCleanup(setattr, storage_worker, 'PREVIEW', storage_worker.DATA_ROOT / 'preview')
